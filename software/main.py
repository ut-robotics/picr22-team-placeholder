...
class Robot:
    ...
    def drive_to_object(self, object_x, object_dist):
        ...
        y_speed = -self.max_speed * y_delta * 0.003
        y_sign = np.sign(y_speed)
        rot_speed = -1 * rot_delta * 0.003
        y_speed = np.clip(abs(y_speed), 0.2, self.max_speed) * y_sign
        rot_speed = np.clip(rot_speed, -self.max_speed, self.max_speed) * -1
        thrower_speed = 0
        self.logger.log.info(f"--Drive2Object-- Y_speed {y_speed}")
        if y_speed < 0:
